"""
Node that implements a kalman filter and implements the PI controller accordingly.
Operation:

=> The sdf will be arranged in a way that it has white noise in its IMU data. 
=> The Kalman filter will be applied to the angular orientation of the pendulum. 
=> The sensor will publish information at a certain rate. 
=> The model x_dot = Ax + Bu will estimate at times when sensor info is not available.
=> The covariances of both measurements will be set based on trial. (IMU's can be figured out online)
=> The covariance updates will be applied when the IMU measurements are available.  
=> The PI controller will act on the filtered sensor information. 
